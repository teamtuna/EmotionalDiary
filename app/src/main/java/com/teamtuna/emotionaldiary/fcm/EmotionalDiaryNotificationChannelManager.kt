...
object EmotionalDiaryNotificationChannelManager {
    ...
    enum class NotificationChannelDefine(var groupId: String, var groupName: String, val channelName: String, var importance: Int, var description: String, var lightColor: Int, var lockScreenVisibility: Int) {
        Emotional(GROUP_ID, GROUP_NAME, "Common Channel", NotificationManager.IMPORTANCE_DEFAULT, "기본체널(Common Channel)", Color.BLUE, Notification.VISIBILITY_PUBLIC),
        Diary(GROUP_ID, GROUP_NAME, "Diary  Channel", NotificationManager.IMPORTANCE_HIGH, "Diary Channel 입니다.", Color.RED, Notification.VISIBILITY_SECRET),
    }
    ...
    fun createNotificationChannel(context: Context) {
        ...
        assert(context.resources.getString(R.string.default_notification_channel_name) == defaultChannel.channelName)
    ...
    fun deleteNotificationChannel(context: Context, channel: NotificationChannelDefine) {
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.O)
            return
        context.getSystemService(NotificationManager::class.java)?.deleteNotificationChannel(channel.name)
    }
}
